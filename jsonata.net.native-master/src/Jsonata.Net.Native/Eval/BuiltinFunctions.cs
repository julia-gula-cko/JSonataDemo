...
namespace Jsonata.Net.Native.Eval
{
    internal static class BuiltinFunctions
    {
        ...
        */
        ...
        #region Base-32-Decode

        /**
          $base32decode()
          Signature: $base32decode()
          Converts string to base 32 bytes , using a UTF-8 Unicode codepage.        
         */
        public static string base32decode([AllowContextAsValue][PropagateUndefined] string str)
        {
            var id = Base32.Decode(str);
            var result = new Guid(id).ToString();
            return result;
        }

        #endregion
        
        private static readonly Dictionary<string, (int id, string name)> acquirers = new Dictionary<string, (int id, string name)>
        {
            { "cko-visa", (22, "Checkout VISA") },
            { "cko-mastercard", (23, "Checkout MC") },
            { "cko-amex", (19, "Checkout AMEX") },
            { "cp_discover", ( 0, "Checkout Discover") },
            { "cp_amex", ( 0, "Checkout AMEX") }
        }; 
    
        #region Try-Get-Acquirer
        /**
          $trygetacquirerId()
          Signature: $trygetacquirerId()
          Mapping Acquirer.        
         */
        public static int trygetacquirerId([AllowContextAsValue][PropagateUndefined] string acquirerId)
        {
            acquirerId.ThrowIfNull();

            if (acquirerId.StartsWith("cp_", StringComparison.Ordinal))
            {
                if (acquirers.ContainsKey(acquirerId))
                {
                    return acquirers[acquirerId].id;
                }
                var acquirerName = acquirerId.Split('_')[1].ToUpper();
               return  0;
               
            }

            acquirerId = acquirerId.Split("_")[0];
            if (acquirers.ContainsKey(acquirerId))
            {
              var result =  acquirers[acquirerId];
              return result.id;
            };
            return 0;
        }
        ...
        /**
          $trygetacquirerName()
          Signature: $trygetacquirerName()
          Mapping Acquirer.        
         */
        public static string?  trygetacquirerName([AllowContextAsValue][PropagateUndefined] string acquirerId)
        {
            ...
            if (acquirerId.StartsWith("cp_", StringComparison.Ordinal))
            {
                if (acquirers.ContainsKey(acquirerId))
                {
                    return acquirers[acquirerId].name;
                }
                var acquirerName = acquirerId.Split('_')[1].ToUpper();
                return  ($"Checkout {acquirerName}");
        ...
        public static string GetGuid([AllowContextAsValue][PropagateUndefined] string value)
        {
            return Id.GetGuid(value).ToString();
        }
